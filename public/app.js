...
async function deleteUploads(ids){
  ...
  try{
    ...
    await Promise.allSettled([loadUploadHistory(),loadOverview(),loadServices(),loadEndpoints(),searchLogs(1),loadAlertsOps(),loadUploadHistory()]);
  }catch(e){toast(e.message,'error');}
}
...
async function refreshAll(){$('tenantEnvChip')&&($('tenantEnvChip').textContent=state.environment);renderEnvButtons();await Promise.allSettled([loadOverview(),loadServices(),loadEndpoints(),searchLogs(1),loadAlertsOps(),loadUploadHistory()]);}
